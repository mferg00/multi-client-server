...
get_mode(char *line)
{
    ...
    else if ((strcmp(line, "SUB") == 0) && argc == 2)
    {
        return SUB;
    }

    // CHANNELS
    else if ((strcmp(line, "CHANNELS") == 0) && argc == 1)
    {
        return CHANNELS;
    }

    // UNSUB
    else if ((strcmp(line, "UNSUB") == 0) && argc == 2)
    {
        return UNSUB;
    }
    ...
    else if ((strcmp(line, "SEND") == 0) && argc == 3)
    {
        return SEND;
    }
...
execute_client(char* buff, size_t buff_size)
{
    ...
    else if(mode == LIVEFEED_ALL)
    {
        ...
        snprintf(buff, buff_size, _TALK _MAG "not subscribed to any channels");
    ...
    else if (mode == NEXT_ALL)
    {
        ...
        while(next_loop_count < MAX_CHANNELS)
        {
            ...
            {
                flag = 0;
                char* msg = next_message(next_loop_count, &err);
                if(err == 0)
                {
    ...
    else if(mode == SEND)
    {
        ...
        {
            case -1:
              snprintf(buff, buff_size, _TALK _RED "message size too long");
              break;

            case -2:
              snprintf(buff, buff_size, _TALK _RED "message buffer full");
              break;
        }
    }
